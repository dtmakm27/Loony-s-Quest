...
function AniisPlaying()
{
	if(use_3d_animation)
	{
		return animation_3d_component.character_pointer.GetComponent.<Animation>().isPlaying;
	}
	else
	{
...
function Start()
{
	GetComponent.<Rigidbody>().sleepVelocity = 0; // a good player never sleeps!
...
if(!player_can_move)return;
	...
	if(current_mode==playerStates.Stopping) // we are stopping
	{
		...
		GetComponent.<Rigidbody>().velocity.x = current_speed*movement_direction*-1;
		return;
	}
    ...
    for (var contact : ContactPoint in collisionInfo.contacts) {
		...
		if(GetComponent(CapsuleCollider))
		{
			if(transform.InverseTransformPoint(contact.point).x<-0.2)
			{
				...
				GetComponent.<Rigidbody>().velocity.y = last_velocity.y;
			}
			if(transform.InverseTransformPoint(contact.point).x>0.2)
			{
				wall_right=true;
