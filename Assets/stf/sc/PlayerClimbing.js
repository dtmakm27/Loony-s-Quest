...
function change_colission(bool : boolean)
{
		...
		{
			Physics.IgnoreCollision(GetComponent.<Collider>(),obj.GetComponent.<Collider>(), bool);
			if(bool)
			{
				obj.layer=2;
			}
...
if(!playerMovement.player_can_move || playerMovement.current_mode==playerStates.Hit)return;
	...
	if(jumping_component) jumping_component.jumped=0;
		...
		GetComponent.<Rigidbody>().velocity.y = playerMovement.return_real_axis().y*climb_speed;
		if(locked_x)
		{
			...
			GetComponent.<Rigidbody>().velocity.x = playerMovement.return_real_axis().x*climb_speed;
		if(transform.position.y > climb_object.position.y+climb_object.GetComponent.<Collider>().bounds.size.y/2 && !cant_unclimb)
		{
		...
		if(jump_while_climbing && jumping_component)
		{
			if(Input.GetButtonDown(jumping_component.jump_button_name) && !cant_unclimb)
			{
				...
				playerMovement.current_speed = GetComponent.<Rigidbody>().velocity.x;
