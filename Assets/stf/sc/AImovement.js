...
function Update () {
		...
		if(current_mode!=aiStates.Dead)
		{
			...
			if(spawn_items.length>0)
			{
				...
				for(var i : int = 0; i<spawn_items.length; i++)
				{
					...
					Physics.IgnoreCollision(pointer.GetComponent.<Collider>(),GetComponent.<Collider>());
				}
			}
		}
		else
	...
	{
		GetComponent.<Rigidbody>().velocity.x=direction*walk_speed;
	...
	{
		GetComponent.<Rigidbody>().velocity.x=direction*run_speed;
	}
	...
	if(attack_target)
	{
		...
		{
			GetComponent.<Rigidbody>().velocity.x=GetComponent.<Rigidbody>().velocity.x/slow_down_by;
		}
