...
using System.IO;
public class urocs : MonoBehaviour {
	public GUIStyle st1;
	public GUIStyle st2;
	public GUIStyle st3;
	...
	void Start () {
		//kopythis
		myLight = GetComponent<debuge>();

		kodirane = System.IO.File.ReadAllText (@"C:\Loony's quest\tasks\ur1\ur1.cpp");
		...
		System.IO.File.Delete(@"C:\Loony's quest\tasks\ur1\ur1.001.txt");
		System.IO.File.Delete(@"C:\Loony's quest\tasks\ur1\ur1.002.txt");
	...
	void Sravni () {
		if (GUI.Button (new Rect (Screen.width/1.9f,Screen.height/1.19f,Screen.width/8f,Screen.height/3f), q,st1)) {
			...
			Process proc = null;
			try {
				string targetDir = string.Format (@"C:\Loony's quest\tasks\ur1");
				proc = new Process ();
			...
			string otgovor1 = System.IO.File.ReadAllText (@"C:\Loony's quest\tasks\ur1\ur1.001.sol");
			string izhod1 = System.IO.File.ReadAllText (@"C:\Loony's quest\tasks\ur1\ur1.001.txt");
	...
	void OnGUI(){ 
		...
		if (tik1.activeSelf == true) {
			...
			if(GUI.Button(new Rect(Screen.width/1.14f,Screen.height/50f,Screen.width/8f,Screen.height/4),s,st1)){
				Application.LoadLevelAdditiveAsync("ur2");
				Destroy (vsichko);
		...
		if (GUI.Button (new Rect (Screen.width/1.5f,Screen.height/1.275f,Screen.width/5f,Screen.height/3),d,st1)) {
			System.IO.File.WriteAllText (@"C:\Loony's quest\tasks\ur1\ur1.cpp", kodirane);
			System.IO.File.Delete(@"C:\Loony's quest\tasks\ur1\ur1.001.txt");
			System.IO.File.Delete(@"C:\Loony's quest\tasks\ur1\ur1.002.txt");
			//greshka = false;
			System.IO.File.Delete (@"C:\Loony's quest\tasks\ur1\a.txt");
			...
			debuginfo = System.IO.File.ReadAllText (@"C:\Loony's quest\tasks\ur1\a.txt");
