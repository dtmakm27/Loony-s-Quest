...
using System.IO;
public class novokomp112 : MonoBehaviour {
	public int b = 4;
	string debuginfo;
	public GUIStyle st4;
	private debuginglgog myLight;
	bool greshka = true;
	...
	void Start () {
		...
		myLight = GetComponent<debuginglgog>();
	...
	void Sravni () {
		if (GUI.Button (new Rect (Screen.width / 1.9f, Screen.height / 1.19f, Screen.width / 8f, Screen.height / 3f), q, st1)) {
			...
			Process proc = null;
			try {
				string targetDir = string.Format (@"C:\Loony's quest\tasks\atm");
				proc = new Process ();
			...
			string otgovor1 = System.IO.File.ReadAllText (@"C:\Loony's quest\tasks\atm\atm.001.sol");
			string izhod1 = System.IO.File.ReadAllText (@"C:\Loony's quest\tasks\atm\atm.001.txt");
	...
	void OnGUI(){ 
		...
		if (GUI.Button (new Rect (Screen.width/1.5f,Screen.height/1.275f,Screen.width/5f,Screen.height/3),d,st1)) {
			System.IO.File.WriteAllText (@"C:\Loony's quest\tasks\atm\atm.cpp", kodirane);
			System.IO.File.Delete(@"C:\Loony's quest\tasks\atm\atm.001.txt");
			System.IO.File.Delete(@"C:\Loony's quest\tasks\atm\atm.002.txt");
			//greshka = false;
			System.IO.File.Delete (@"C:\Loony's quest\tasks\atm\a.txt");
			...
			debuginfo = System.IO.File.ReadAllText (@"C:\Loony's quest\tasks\atm\a.txt");
