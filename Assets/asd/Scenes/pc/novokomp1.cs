...
using System.IO;
public class novokompi1 : MonoBehaviour {
	public int b = 4;
	public int c = 6;
	...
	void OnGUI(){ 
		GUI.Box (new Rect (50,50,1570,760), "n");
		
	GUI.TextField(new Rect(20,30,500,600), kodaji);

				GUILayout.BeginArea (new Rect (Screen.width / b, Screen.height / c, 400, 500));
				kodirane = GUILayout.TextArea (kodirane);
				GUILayout.EndArea ();
		
		
				if (GUI.Button (new Rect (1100, 70, 150, 150), "Kompilirai")) {
		...
		if (GUI.Button (new Rect (1250, 70, 150, 150), "Sravni")) {
	...
	void Update () {
		if (hiks1.activeSelf == true) {
			...
			hinta.gameObject.SetActive (false);
