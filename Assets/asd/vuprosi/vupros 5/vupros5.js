...
var men: GUIStyle;
	function OnGUI () {
		...
		windowRect = GUI.Window (0,rect,DoMyWindow, "Въпрос#5");
	}
	// Make the contents of the window
	function DoMyWindow (windowID : int) {
	GUI.Label(rect3,"Дробен тип данни е:",men);
	
		GUILayout.BeginArea(Rect(rect2));
		if(GUILayout.Button("int", GUILayout.Height(buttonHeight)) || Input.GetKeyUp ("z")){
		...
		if(GUILayout.Button("float", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("x")) {
		...
		GUILayout.Space(spacing);
		if(GUILayout.Button("uint", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("c")) {
		Application.LoadLevelAdditiveAsync("greshno");
			Destroy (this.gameObject);
			Destroy (other);
			Destroy (another);
