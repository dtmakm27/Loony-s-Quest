...
var men: GUIStyle;
	function OnGUI () {
		...
		windowRect = GUI.Window (0,rect,DoMyWindow, "Въпрос#7");
	}
	// Make the contents of the window
	function DoMyWindow (windowID : int) {
	GUI.Label(rect3,"Знакът = на езикът C++ означава:",men);
	
		GUILayout.BeginArea(Rect(rect2));
		if(GUILayout.Button("равно", GUILayout.Height(buttonHeight)) || Input.GetKeyUp ("z")){
		...
		if(GUILayout.Button("присвояване", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("x")) {
		...
		GUILayout.Space(spacing);
		if(GUILayout.Button("не е равно", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("c")) {
		Application.LoadLevelAdditiveAsync("greshno");
			Destroy (this.gameObject);
			Destroy (other);
			Destroy (another);
