...
var men: GUIStyle;
	function OnGUI () {
		...
		windowRect = GUI.Window (0,rect,DoMyWindow, "Въпрос#9");
	}
	// Make the contents of the window
	function DoMyWindow (windowID : int) {
	GUI.Label(rect3,"Що е цикъл ?",men);
	
		GUILayout.BeginArea(Rect(rect2));
		if(GUILayout.Button("условен оператор", GUILayout.Height(buttonHeight)) || Input.GetKeyUp ("z")){
		...
		if(GUILayout.Button("повтарящо се действие", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("x")) {
		...
		GUILayout.Space(spacing);
		if(GUILayout.Button("променливаи", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("c")) {
		Application.LoadLevelAdditiveAsync("greshno");
			Destroy (this.gameObject);
			Destroy (other);
			Destroy (another);
