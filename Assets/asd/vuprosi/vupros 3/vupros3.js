...
var men: GUIStyle;
	function OnGUI () {
		...
		windowRect = GUI.Window (0,rect,DoMyWindow, "Въпрос#3");
	}
	// Make the contents of the window
	function DoMyWindow (windowID : int) {
	GUI.Label(rect3,"cin>> и cout<< са:",men);
	
		GUILayout.BeginArea(Rect(rect2));
		if(GUILayout.Button("променливи", GUILayout.Height(buttonHeight)) || Input.GetKeyUp ("z")){
		...
		if(GUILayout.Button("оператори", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("x")) {
		...
		GUILayout.Space(spacing);
		if(GUILayout.Button("алгоритми", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("c")) {
		Application.LoadLevelAdditiveAsync("greshno");
			Destroy (this.gameObject);
			Destroy (other);
			Destroy (another);
