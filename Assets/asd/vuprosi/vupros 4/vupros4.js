...
var men: GUIStyle;
	function OnGUI () {
		...
		windowRect = GUI.Window (0,rect,DoMyWindow, "Въпрос#4");
	}
	// Make the contents of the window
	function DoMyWindow (windowID : int) {
	GUI.Label(rect3,"Как започва една програмa на С++?",men);
	
		GUILayout.BeginArea(Rect(rect2));
		if(GUILayout.Button("#include <iostream>", GUILayout.Height(buttonHeight)) || Input.GetKeyUp ("z")){
		...
		if(GUILayout.Button("using namespace std", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("x")) {
		...
		GUILayout.Space(spacing);
		if(GUILayout.Button("int main()", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("c")) {
		Application.LoadLevelAdditiveAsync("greshno");
			Destroy (this.gameObject);
			Destroy (other);
			Destroy (another);
