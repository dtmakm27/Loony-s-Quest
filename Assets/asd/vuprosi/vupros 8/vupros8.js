...
var men: GUIStyle;
	function OnGUI () {
		...
		windowRect = GUI.Window (0,rect,DoMyWindow, "Въпрос#8");
	}
	// Make the contents of the window
	function DoMyWindow (windowID : int) {
	GUI.Label(rect3,"if-else са оператори за:",men);
	
		GUILayout.BeginArea(Rect(rect2));
		if(GUILayout.Button("въпрос", GUILayout.Height(buttonHeight)) || Input.GetKeyUp ("z")){
		...
		if(GUILayout.Button("равенство", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("x")) {
			Application.LoadLevelAdditiveAsync("greshno");
			Destroy (this.gameObject);
			Destroy (other);
			Destroy (another);
		...
		GUILayout.Space(spacing);
		if(GUILayout.Button("условие", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("c")) {
