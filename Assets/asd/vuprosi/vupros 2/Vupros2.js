...
var men: GUIStyle;
	function OnGUI () {
		...
		windowRect = GUI.Window (0,rect,DoMyWindow, "Въпрос#2");
	}
	// Make the contents of the window
	function DoMyWindow (windowID : int) {
	GUI.Label(rect3,"Единственият език който компютарът разбира е:",men);
	
		GUILayout.BeginArea(Rect(rect2));
		if(GUILayout.Button("С++", GUILayout.Height(buttonHeight)) || Input.GetKeyUp ("z")){
		...
		if(GUILayout.Button("C#", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("x")) {
			Application.LoadLevelAdditiveAsync("greshno");
			Destroy (this.gameObject);
			Destroy (other);
			Destroy (another);
		...
		GUILayout.Space(spacing);
		if(GUILayout.Button("нули и единици", GUILayout.Height(buttonHeight))|| Input.GetKeyUp ("c")) {
